...
class ApplicationHelper extends BaseRegistry
{
    ...
    public static function get($key, $defailt = null)
    {
        if (self::has($key)) {
    ...
    public static function remove($key)
    {
        if (self::has($key) && self::hasLock($key)) {
            ...
            throw new MyException(
                "Удаление невозможно! "
                . "Переменная регистра не существует "
    ...
    private static function hasLock($key)
    {
    ...
    public function init($startConfig)
    {
        ...
        if (is_array($startConfig)) {
            ...
            self::ensure(
                is_array($startConfig),
                "Передайте верный параметр начальной конфигурации"
            );
        }
    }
    ...
    private static function getOptions($startConfig)
    {
        ...
        self::ensure(
            is_array($startConfig['system_config']),
        ...
        self::ensure(
            is_array($startConfig['default_config']),
            "Не найден раздел контроллера \"По умолчанию\""
        );
        self::set('default_config', $startConfig['default_config']);
        $loader = new Psr4AutoloaderClass();
        $loader->register();
        $loader->addNamespace(
            $startConfig['system_config']['system_vendor'].'\base',
            $startConfig['system_config']['system_folder'].'/base'
        );
        $loader->addNamespace(
            $startConfig['system_config']['system_vendor'].'\MVC',
            $startConfig['system_config']['system_folder'].'/MVC'
        );
        self::ensure(
            is_array($startConfig['user_config']),
            "Системная конфигурация повреждена"
        );
